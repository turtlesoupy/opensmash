...
Optional particles
trails=[] emits none. Each trail names an authored glyph and the ZERO-BASED index of a hit in that same move's hits array. With one hit, the only valid trail.hit is 0; with two hits, the valid indices are 0 and 1. The index is not a move number, damage value, or frame. The schema bounds count, lifetime, spread, and drift; choose values for the action. The runtime gives particles independent birth positions, inherited source velocity, gravity, connected rotation, and fading, and ends them by the move's duration. Authored glyph size is preserved. Particles must not seek the opponent or imply contact that did not occur.

Realizing the frozen action
Preserve the description's retrieval, appearance, handling, and emission points in body poses and prop trajectories. The fighter root is a coordinate reference, not a default visual spawn point. Held props should stay attached to the intended grip as the character moves. Emitted effects should begin at the relevant body part, prop, or contact point. Releases and transfers should be deliberate and visually continuous. Coordinate body poses, prop trajectories, and effect origins throughout the action; using a plausible starting offset alone does not maintain attachment. Keep the origin readable in both contexts; do not relocate an explicitly intentional bodily effect or silently redesign a frozen description.
Treat the description's defining object and action verbs as visible requirements. A prop ID or palette alone does not depict an object. Build its identifying silhouette and a few separated landmarks from geometry; preserve those landmarks through the motion. Spend the available pieces on recognition before trim. Put contrasting detail layers at distinct increasing z offsets so opaque pieces do not hide one another.
Use the 350-unit fighter as a measuring reference. At a wide match camera that fighter can occupy only about 80 screen pixels: a 4-unit-wide detail is under one pixel, and an 80-unit-wide main prop is only about 18 pixels. Choose the dominant prop's overall dimensions relative to the fighter and its described role, then give its identifying details enough area to survive that reduction. This is a scale reference, not a minimum size for every object or a requirement to add effects. Keep held objects in a readable relationship to the hands and torso rather than hiding them at the root; on a floor impact the upper silhouette still needs to clear the floor and foreground clutter.
Express anticipation, decisive travel, and follow-through using visibly different body poses and prop positions. A described broad attack needs both broad visible coverage and matching collision. A described squash or rebound needs distinct assembly scale and position keys; a tiny vertical translation cannot substitute for that action. Supporting weapon assemblies end at the first active frame and resume at the last active frame, with matching pose and full scale at each handoff. Add interior keys to give important objects a visible hold before the automatic final-segment fade. Active prop geometry currently stays rigid during each hit, so place compression and spring-back at the adjacent assembly beats.
Preserve explicitly described fragments as well as the dominant object; choosing prop presentation does not replace the described fragments. When the description includes emitted fragments, give their shapes sufficient thickness, contrast, visible clearance and travel to be noticed at that same camera. They may be sparse; they must not disappear inside the body or immediately below the floor. Do not add particles to a description that does not need them.
