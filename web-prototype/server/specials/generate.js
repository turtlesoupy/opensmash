...
export function createModel({apiKey=process.env.OPENAI_API_KEY,model=process.env.SPECIALS_MODEL||'gpt-6-astra',fetchImpl=fetch}={}) {
  return async ({instructions,input,schema,name,signal})=>{
    ...
    const response=await fetchImpl('https://api.openai.com/v1/responses',{
      ...
      body:JSON.stringify({model,store:false,instructions,input:input.referenceImage ? [{role:"user",content:[{type:"input_text",text:JSON.stringify({...input,referenceImage:undefined})},{type:"input_image",image_url:input.referenceImage}]}] : JSON.stringify(input),max_output_tokens:24000,
    ...
    return {value:JSON.parse(text),provenance:{model:result.model,responseId:result.id,usage:result.usage}};
  };
}

// Checkpoint the description before calling the implementer. Resume never rewrites it.
export async function generateSet({character,profile,model=createModel(),checkpoint=async()=>{},signal,referenceImage=null,brief:existingBrief=null}) {
  signal?.throwIfAborted();
  const written=existingBrief ? {value:existingBrief,provenance:{resumed:true}} : await model({instructions:PRINCIPLES,input:{character,contexts:SLOTS,profile,...(referenceImage?{referenceImage}:{})},schema:briefSchema,name:'special_description',signal});
  ...
  signal?.throwIfAborted();
  const built=await model({instructions:IMPLEMENT,input:{character,profile,brief,briefHash},schema:implementationSchema,name:'special_implementation',signal});
  await checkpoint('implementation',{implementation:built.value,briefHash,provenance:built.provenance});
  signal?.throwIfAborted();
  const packet=compileSet({brief,implementation:built.value,profile,character});
  if(hash(brief)!==briefHash) throw new Error('frozen description changed');
  const report={status:'compiled',contexts:SLOTS.map(slot=>({slot,compiled:true})),judges:0,visualQuality:'not-assessed',runtimeValidated:false,briefHash,packageHash:hash(packet)};
